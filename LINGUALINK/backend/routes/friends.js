...
import { checkAuth } from "./auth.js"; // your JWT middleware
...
router.post("/request", checkAuth, async (req, res) => {
  try {
    ...
    if (receiver.friends.includes(senderId)) return res.status(400).json({ error: "Already friends" });
...
router.post("/accept", checkAuth, async (req, res) => {
  try {
    ...
    if (!user.friendRequests.includes(requesterId)) return res.status(400).json({ error: "No such friend request" });
